...
public class MainApp {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        while (readFile.hasNext()) {
            ...
            Echipament.Situatie sit = Echipament.Situatie.valueOf(situatie);
            String tip = date[5];
            ...
            switch (tip_e) {
                case imprimanta:
                    ...
                    String mod_t = date[9];
                    Imprimante.Tiparire mod = Imprimante.Tiparire.valueOf(mod_t);

                    Imprimante imp = new Imprimante(denum, nr_i, pret, zona, sit, tip_e, pp, dp, nrp, mod);
                    lista.add(imp);
                    break;
                case copiator:
                    int nr_p = Integer.parseInt(date[6]);
                    String form = date[7];
                    Copiatoare.Format f = Copiatoare.Format.valueOf(form);

                    Copiatoare cop = new Copiatoare(denum, nr_i, pret, zona, sit, tip_e, nr_p, f);
                    lista.add(cop);
                    break;
                ...
                case sistem_de_calcul:
                    ...
                    String sist = date[9];
                    Sistem_calcul.Sistem_operare so = Sistem_calcul.Sistem_operare.valueOf(sist);

                    Sistem_calcul s = new Sistem_calcul(denum, nr_i, pret, zona, sit, tip_e, monitor, viteza, cap, so);
                    lista.add(s);
                    break;
            }
            ...
            switch (optiune) {
                ...
                case 6:
                    ...
                    for(Echipament e: lista)
                    {
                        if(e.getDenumire().compareTo(denumire_i)==0) {
                            ok = 1;
                            System.out.println("Modul de tiparire al imprimantei selectate este: "+((Imprimante) e).getMod_tiparire());
                ...
                case 7:
                    ...
                    for(Echipament e:lista)
                    {
                        ...
                        {
                            ok=1;
                            System.out.println("Formatul de copiere al copiatorului selectat este: "+((Copiatoare) e).getFormat());
                ...
                case 8:
                    ...
                    for(Echipament e:lista)
                    {
                        ...
                        {
                            ok=1;
                            System.out.println("Sistemul de operare al sistemului de calcul selectat este: "+((Sistem_calcul) e).getSistem());
                            ...
                            ((Sistem_calcul) e).setSistem(sist);
                            System.out.println(e);
                        }
                    }
                    if(ok==0)
